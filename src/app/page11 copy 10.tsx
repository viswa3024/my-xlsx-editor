...
export default function Home() {
  ...
  const sheetEditableConfig: Record<string, Record<string, string>> = {
    Risk: {
      "Risk Title": "input",
      "Risk Description": "textarea",
      Probability: "selectHighLow",
      Impact: "selectHighLow",
      "Response Plan": "selectTrueFalse",
    },
    Issue: {
      "Issue Title": "input",
      "Issue Description": "textarea",
      Severity: "selectHighLow",
      Status: "selectOpenClosed",
    },
  };
  ...
  const updateJsonValue = (header: string, rowIndex: number, value: any) => {
    ...
    const colIndex = data[0].findIndex((h) => h === header);
    if (colIndex >= 0) {
  ...
  return (
    <main className="p-6">
      <h1 className="text-2xl font-bold mb-4">📊 XLSX Editor (Sheet Specific Editable Columns)</h1>
            ...
            <table className="border-collapse w-full">
              <tbody>
                {sheets[activeSheet].data.map((row, rIdx) => {
                  ...
                  const editableConfig = sheetEditableConfig[sheetName] || {};
                  ...
                  return (
                    <tr key={rIdx}>
                      {row.map((cell, cIdx) => {
                        ...
                        const editType = editableConfig[headerName];
                        const isEditable = !isHeader && !!editType;
                            ...
                            {isEditable ? (
                              editType === "input" ? (
                                <input
                                  ...
                                  value={sheetJson[headerName]?.[rIdx - 1] ?? ""}
                                  onChange={(e) => updateJsonValue(headerName, rIdx - 1, e.target.value)}
                                />
                              ) : editType === "textarea" ? (
                                <textarea
                                  ...
                                  onChange={(e) => updateJsonValue(headerName, rIdx - 1, e.target.value)}
                                />
                              ) : editType === "selectHighLow" ? (
                                <select
                                  className="w-full border-none p-1 focus:outline-none"
                                  value={sheetJson[headerName]?.[rIdx - 1] ?? "Low"}
                                  onChange={(e) => updateJsonValue(headerName, rIdx - 1, e.target.value)}
                                >
                                  <option value="High">High</option>
                                  <option value="Low">Low</option>
                                </select>
                              ) : editType === "selectTrueFalse" ? (
                                <select
                                  className="w-full border-none p-1 focus:outline-none"
                                  value={sheetJson[headerName]?.[rIdx - 1] ?? "FALSE"}
                                  onChange={(e) => updateJsonValue(headerName, rIdx - 1, e.target.value)}
                                >
                                  <option value="TRUE">TRUE</option>
                                  <option value="FALSE">FALSE</option>
                                </select>
                              ) : editType === "selectOpenClosed" ? (
                                <select
                                  className="w-full border-none p-1 focus:outline-none"
                                  value={sheetJson[headerName]?.[rIdx - 1] ?? "Open"}
                                  onChange={(e) => updateJsonValue(headerName, rIdx - 1, e.target.value)}
                                >
