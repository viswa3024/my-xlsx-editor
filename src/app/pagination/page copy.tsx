...
export default function Home() {
  ...
  const totalRows = sheets.length > 0 ? sheets[activeSheet].data.length - 1 : 0;
  const totalPages = Math.ceil(totalRows / rowsPerPage);

  const paginatedRows = sheets.length > 0 
    ? [sheets[activeSheet].data[0], ...sheets[activeSheet].data.slice((currentPage - 1) * rowsPerPage + 1, currentPage * rowsPerPage + 1)]
    : [];

  // Generate page numbers with skip ...
                  ...
                  return (
                    <tr key={rIdx}>
                      {row.map((cell, cIdx) => {
                        ...
                        if (headerName === "Payment") return null;
                        ...
                        return (
                          <td
                            ...
                            colSpan={colSpan}
                            style={{
                              width: headerWidths[cell as string] || "auto",
                            }}
                          >
                            {isHeader ? (
                              displayHeader
